...
import { extractSheetId, fetchGoogleSheet } from "@/utils/googleSheets";
...
export function GoogleSheetsConnect({
  ...
  const [autoPoll, setAutoPoll] = useState(
    (activeConfig?.pollInterval || 0) > 0
  );
  const [pollInterval, setPollInterval] = useState(
    activeConfig?.pollInterval || 30
  );
  const [lastFetched, setLastFetched] = useState<number | null>(
    activeConfig?.lastFetched || null
  );
  ...
  const fetchSheet = useCallback(
    ...
    [url, activeConfig, googleApiKey, autoPoll, pollInterval, onSheetLoaded]
  );
  ...
  return (
    <div className="rounded-lg border border-dashed border-primary/30 bg-card p-5 space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        Paste a Google Sheets URL. Sheet must be shared as "Anyone with the
        link"
        {googleApiKey
          ? " or use your API key for private sheets."
          : "."}
      </p>
      <Input
        value={url}
